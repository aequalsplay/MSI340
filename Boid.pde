...
Object obj;
  ...
  Boid (PVector position, DNA dna_) {   
     ...
     red = map(dna.genes[0], 0, 1, 40, 255);
     green = map(dna.genes[1], 0, 1, 40, 255); 
     blue = map(dna.genes[2], 0, 1, 40, 255);
     println(dna.genes[1]);
     ...
     // OSC Address
     count = floor(map(dna.genes[0], 0, 1, 1, 20));
     oscAddr = "/boid" + str(count);
     print(oscAddr);
  ...
  void display() {
     ...
     float theta = vel.heading() + radians(90);
       fill(red, green, blue, life);
       stroke(red, green, blue, life+100);
       strokeWeight(8);
  ...
  // Avoid main object
  void avoidObj(float mult){    
    float d = PVector.dist(object.pos, pos);
    if(d < r + 100) {
      PVector desired = PVector.sub(pos, object.pos);
      PVector avoidObj = PVector.sub(desired, vel);
      avoidObj.mult(mult);
      acc.mult(0);
      applyForce(avoidObj);
    }    
 ...
 // Render wandering circle, etc.
   ...
   // OSC STUFF  
    ...
    OscMessage oscBoid = new OscMessage(oscAddr);
    //println(oscStuff);
     
    oscBoid.add(boidPosX);
    oscBoid.add(boidPosY);
    oscBoid.add(boidVel);
    oscBoid.add(boidR);
    osc.send(oscBoid, supercollider);
